...
import { loginGuest } from "../services/authApi";
import { useNavigate } from 'react-router-dom';


export function GuestLogin({ onLoginSuccess }) {
...
const handleSubmit = async (e) => {
  ...
  setLoading(true);
  try {
    const response = await loginGuest(form);
    onLoginSuccess(response);
  } catch(err) {
    setMessage(err.response?.response?.message || "Login failed");
  } finally {
    setLoading(false);
  }
